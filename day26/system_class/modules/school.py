...
class School(object):
    ...
    def show_grade(self):
        for key in self.school_grade:
            ...
            print('\033[32;1m课程【%s】\t课程【%s】\033[0m' %(grade_obj.grade_name, grade_obj.course_obj.course_name))
    ...
    def show_teacher(self):
        for key in self.school_teacher:
            ...
            grade_list = []
            for grade_name in teacher_obj.teacher_grade:
                grade_list.append(grade_name)
            
            print('\033[32;1m讲师【%s】\t薪资【%s元】\t关联的班级列表【%s】\033[0m' % (teacher_obj.teacher_name, 
                                                                     teacher_obj.teacher_salary, grade_list))

    def show_grade_course(self):
        for key in self.school_grade:
            grade_obj = self.school_grade[key]
            course_obj = grade_obj.course_obj
            print('\033[32;1m班级【%s】\t课程【%s】\t费用【%s元】\t周期【%s个月】\033[0m' %(
    ...
    def show_grade_student(self, teacher_name):
        teacher_obj = self.school_teacher[teacher_name]
        for grade_name in teacher_obj.teacher_grade:
            grade_obj = teacher_obj.teacher_grade[grade_name]
            st_list = []
            for st_name in grade_obj.grade_student:
                st_list.append(st_name)
            print('\033[32;1m班级【%s】\t课程【%s】\t学员列表【%s】\033[0m' %(
                grade_name, grade_obj.course_obj.course_name, st_list))
