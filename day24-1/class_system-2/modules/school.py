...
class School(object):
    ...
    def show_teacher(self):
        for key in self.school_teacher:
            ...
            print('\033[32;1m讲师【%s】\t薪资【%s】\t学生列表【%s】\033[0m' % (
                teacher_obj.teacher_name, teacher_obj.teacher_salary, student_list))
    def show_grade_course(self):
        for key in self.school_grade:
            grade_obj = self.school_grade[key]
            print('\033[32;1m班级【%s】\t课程【%s】\t周期【%s】\t价格【%s】\033[0m' % (
                grade_obj.grade_name, grade_obj.course_obj.course_name,
                grade_obj.course_obj.course_time, grade_obj.course_obj.course_price))
    ...
    def show_teacher_grade(self, teacher_name):
        teacher_obj = self.school_teacher[teacher_name]
        for i in teacher_obj.teacher_grade:
            grade_obj = teacher_obj.teacher_grade[i]
            student_list = []
