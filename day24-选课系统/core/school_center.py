...
class Manage_school(object):
    ...
    def run_manage(self):
        ...
        while flag:
            ...
            choice_school = input('\033[34;1m请选择学校名：\033[0m').strip()
            if not choice_school: continue
            if choice_school in self.db_main:
                print('\033[33;1m欢迎进入【%s】学校\033[0m'.center(50, '#') % choice_school)
                self.school = self.db_main[choice_school]
                while flag:
